...
fn is_pangram(counts: &[u32]) -> bool {
    counts.iter().all(|&x| x > 0) // Ogni lettera deve comparire almeno una volta
}
...
pub fn run_pangram() {
    ...
    let mut counts: [u32; 26] = [0; 26]; // Inizializza l'array

    for line in reader.lines() {
        let line_counts = stats(&line.unwrap());
        for i in 0..26 {
            counts[i] += line_counts[i]; // Somma i conteggi
        }
    }

    if is_pangram(&counts) {
        println!("Il testo è un pangramma!");
    } else {
        println!("Il testo NON è un pangramma!");
    }
}
